...
class ExtensionTwo(Scene):
	...
	def construct(self):
		...
		# Begin Midy's Theorem itself

		MathTex.set_default(font_size=64)

		lines = [
			# MathTex(r"p \in \mathbb{P},p>7"),
			# VGroup(
			# 	MathTex(r"\frac{1}{p}=0.\overline{a_{1}a_{2}a_{3}\ldots a_{k}},k=bn,b,n\in \mathbb{Z}"),
		...
		for m in range(1, len(lines)):
			if str(lines[m])[0] == "M" and str(prev)[0] == "M":
				self.play(
					TransformMatchingTex(
						prev, lines[m], **play_kw
					)
				)
				self.wait(defaultTimeout)
			elif str(lines[m])[0] == "M" and str(prev)[0] == "V":
				self.play(
					Unwrite(
						prev, **play_kw
					)
				...
				self.wait(defaultTimeout)
			elif str(lines[m])[0] == "V" and str(prev)[0] == "M":
				lines[m].arrange(DOWN, buff=0.5)
				
				self.play(
					TransformMatchingTex(
						prev, lines[m][0], **play_kw
					)
				)
				self.wait(defaultTimeout)
				...
				for i in range(1, len(lines[m])):
					self.play(
						TransformMatchingTex(
							lines[m][i - 1].copy(), lines[m][i], **play_kw
						)
					)
					self.wait(defaultTimeout)

			elif str(lines[m])[0] == "V" and str(prev)[0] == "V":
				lines[m].arrange(DOWN, buff=0.5)
